...
TEST(MazeTest, EmptyRows_no11) {
    Maze* maze = maze_create(NULL, 0);
    EXPECT_EQ(maze, nullptr);
}

TEST(MazeTest, InvalidRows_no12) {
    const char* grid[] = { "S", "EX" };
    Maze* maze = maze_create(grid, 2);
    EXPECT_EQ(maze, nullptr);
}

TEST(MazeTest, NullPathPtr_no13) {
    const char* grid[] = { "S E" };
    Maze* maze = maze_create(grid, 1);
    ASSERT_NE(maze, nullptr);
    ...
    EXPECT_FALSE(find_path(maze, start, NULL, &len));
    maze_destroy(maze);
}

TEST(MazeTest, NullPathLenPtr_no14) {
    const char* grid[] = { "S E" };
    Maze* maze = maze_create(grid, 1);
    ASSERT_NE(maze, nullptr);
    ...
    EXPECT_FALSE(find_path(maze, start, &path, NULL));
    maze_destroy(maze);
}

TEST(MazeTest, StartOutOfBounds_no15) {
    const char* grid[] = { "S E" };
    Maze* maze = maze_create(grid, 1);
    ASSERT_NE(maze, nullptr);
...
TEST(MazeTest, LargeMaze_no16) {
    ...
    ASSERT_TRUE(find_path(maze, start, &path, &len));
    EXPECT_GT(len, 1);
    EXPECT_EQ(path[len - 1].x, 9);
    EXPECT_EQ(path[len - 1].y, 15);
