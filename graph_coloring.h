...
// Рекурсивная функция поиска с возвратом, которая пытается покрасить вершины, начиная с v
bool graphColoringRecursive(Graph* g, int* colors, int v, int K);

// Основная функция для внешнего вызова, которую мы будем тестировать. Будет возврашать true, если раскраска будет найдена и запишет цвета в result_colors
...
bool writeResultToFile(const char* filename, const int* colors, int n, bool successs);
