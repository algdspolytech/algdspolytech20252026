...
// Вспомогательная проверка раскраски
...
// 6. TEST(GraphColoringTest, DisconnectedComponents_no10) {
...
// 8. Двудольный (цепочка длины 5) — K=2 должно быть возможно 
TEST(GraphColoringTest, Path5_K2_no7) {
    ...
    EXPECT_TRUE(check_coloring(g, 2, colors, res));
    graph_free(g);
}Несвязный граф: одна компонента раскрашиваемая, другая нет 
...
// 10. Ещё один кейс: цикл нечетной длины 
